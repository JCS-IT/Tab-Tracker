		...
		<v-row id="generate new staff">
			<v-col>
				...
				<v-card class="newUser" width="300" color="white" v-if="showNewUserMenu">
					...
					<v-form>
						<v-text-field type="text" v-model="newUser" placeholder="Enter the users name..." />
						<v-card-actions>
							...
							<v-list>
								<v-list-item>
									<div class="text-center">
										<v-dialog v-model="deleteUserMenu" scrollable fullscreen persistent :overlay="true"
											max-width="300px" max-height="200px" transition="dialog-transition">
											<template v-slot:activator="{ props }">
